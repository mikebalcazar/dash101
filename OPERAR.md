...
curl -s -o /dev/null -w "%{http_code}\n" -H "Authorization: Bearer $T" \
  https://api.github.com/repos/mikebalcazar/conta-master
```
...
```bash
git clone "https://x-access-token:${T}@github.com/mikebalcazar/conta-master.git" /home/claude/repo
...
curl -s -H "Authorization: Bearer $T" \
  "https://api.github.com/repos/mikebalcazar/conta-master/actions/runs?per_page=3" \
...
curl -s -X POST -H "Authorization: Bearer $T" \
  https://api.github.com/repos/mikebalcazar/conta-master/pulls \
  -d '{"title":"…","head":"claude/…","base":"main","body":"…"}'
# y con el número que devuelve:
curl -s -X PUT -H "Authorization: Bearer $T" \
  https://api.github.com/repos/mikebalcazar/conta-master/pulls/<N>/merge \
...
curl -s -H "Authorization: Bearer $T" \
  "https://api.github.com/repos/mikebalcazar/conta-master/commits/<SHA>/comments" \
  | python3 -c "import json,sys; [print(c['body']) for c in json.load(sys.stdin)]"

# y si algo falló, en qué trabajo fue
curl -s -H "Authorization: Bearer $T" \
  "https://api.github.com/repos/mikebalcazar/conta-master/actions/runs/<ID>/jobs" \
...
```bash
curl -s -H "Authorization: Bearer $T" \
  "https://api.github.com/repos/mikebalcazar/conta-master/actions/permissions/workflow"
```
...
```bash
curl -s -X POST -H "Authorization: Bearer $T" \
  "https://api.github.com/repos/mikebalcazar/conta-master/actions/workflows/verificar.yml/dispatches" \
