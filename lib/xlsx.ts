...
export function xlsx(hojas: Hoja[]): Blob {
  const cod = new TextEncoder();
  ...
  return new Blob([zip(partes) as BlobPart], {
    type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
  });
}

/** Bajarlo. Vive aquí para que quien pinta no tenga que acordarse de soltar
 *  la URL: un `createObjectURL` que nadie revoca deja el archivo en memoria
