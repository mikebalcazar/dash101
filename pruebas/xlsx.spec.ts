...
import { columna, crc32, xlsx } from "@/lib/xlsx";
...
describe("el libro", () => {
  it("trae las cuatro piezas del formato y una hoja por pestaña", async () => {
    const partes = abrir(await bytesDe(xlsx([
  ...
  it("los números van como números y los textos como texto", async () => {
    const partes = abrir(await bytesDe(xlsx([{ nombre: "H", filas: [["Puerta", 2850, 25]] }])));
  ...
  it("un «&» y un «<» en el nombre no rompen el archivo", async () => {
    const partes = abrir(await bytesDe(xlsx([{ nombre: "H", filas: [["Puerta A & B <chica>"]] }])));
    const hoja = partes["xl/worksheets/sheet1.xml"];
    expect(hoja).toContain("Puerta A &amp; B &lt;chica&gt;");
    expect(hoja).not.toContain("B <chica>");
  });

  it("una celda vacía no escribe celda, y eso no corre las de al lado", async () => {
    const partes = abrir(await bytesDe(xlsx([{ nombre: "H", filas: [["A", null, "C"]] }])));
    const hoja = partes["xl/worksheets/sheet1.xml"];
    expect(hoja).not.toContain('r="B1"');
    expect(hoja, "la tercera sigue siendo la C").toContain('r="C1"');
  });

  it("el nombre de la pestaña se limpia de lo que Excel no acepta", async () => {
    const partes = abrir(await bytesDe(xlsx([{ nombre: "Obra/2026 [HOLCIM]: pagos y más de treinta y un caracteres", filas: [["x"]] }])));
    const nombre = /name="([^"]*)"/.exec(partes["xl/workbook.xml"])![1];
    expect(nombre.length).toBeLessThanOrEqual(31);
    expect(nombre).not.toMatch(/[\\/?*[\]:]/);
  });

  it("aguanta una obra grande sin despeinarse", async () => {
    ...
    const partes = abrir(await bytesDe(xlsx([{ nombre: "Ítems", filas }])));
